...
namespace PetaPoco
{
	...
	public class Database : IDisposable
	{
			...
			try
			{
				...
				try
				{
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						{
						    if (DoNotInsertIfResultColumn(i)) continue;
								
							if (InsertPrimaryKeyOnlyIfDbIsOracle(primaryKeyName, autoIncrement, i, pd, names, values)) continue;
						...
						cmd.CommandText = CreateInsertCommandText(tableName, names, values);
						...
						object id = ExecuteSqlCommandBasedonDatabaseType(primaryKeyName, cmd);
	    ...
	    private string CreateInsertCommandText(string tableName, List<string> names, List<string> values)
	    {
	    ...
	    private bool InsertPrimaryKeyOnlyIfDbIsOracle(string primaryKeyName, bool autoIncrement, KeyValuePair<string, PocoColumn> i, PocoData pd,
	    ...
	    private static bool DoNotInsertIfResultColumn(KeyValuePair<string, PocoColumn> i)
	    {
	        return i.Value.ResultColumn;
	    }

	    private object ExecuteSqlCommandBasedonDatabaseType(string primaryKeyName, IDbCommand cmd)
	    {
	        object id;
	        switch (_dbType)
	        {
